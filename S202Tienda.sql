...
SELECT * FROM producto p WHERE p.precio = (SELECT MAX(l.precio) FROM producto l LEFT JOIN fabricante f ON l.codigo_fabricante = f.codigo WHERE f.nombre = "Lenovo");

SELECT p.nombre FROM fabricante f LEFT JOIN producto p ON f.codigo = p.codigo_fabricante WHERE f.nombre = "Lenovo" ORDER BY p.precio DESC LIMIT 1;

SELECT p.nombre FROM fabricante f LEFT JOIN producto p ON f.codigo = p.codigo_fabricante WHERE f.nombre = "Hewlett-Packard" ORDER BY p.precio LIMIT 1;

SELECT * FROM producto p WHERE p.precio >= (SELECT MAX(l.precio) FROM producto l LEFT JOIN fabricante f ON l.codigo_fabricante = f.codigo WHERE f.nombre = "Lenovo");

SELECT f.nombre, p.nombre, p.precio FROM fabricante f LEFT JOIN producto p ON f.codigo = p.codigo_fabricante WHERE f.nombre = "ASUS" AND p.precio > (SELECT AVG(a.precio) FROM producto a LEFT JOIN fabricante f ON a.codigo_fabricante = f.codigo WHERE f.nombre = "Asus");
